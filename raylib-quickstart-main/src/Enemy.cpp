...
void Enemy::LogicBrain() {
    ...
    if (IsTileWalkable(Px, Py - 1)) possible.push_back({ 0, -1 });
    if (IsTileWalkable(Px, Py + 1)) possible.push_back({ 0, 1 });  
    if (IsTileWalkable(Px - 1, Py)) possible.push_back({ -1, 0 }); 
    if (IsTileWalkable(Px + 1, Py)) possible.push_back({ 1, 0 });  

    Point reverseMov = { -lastDirection.x, -lastDirection.y };
    possible.erase(
        std::remove(possible.begin(), possible.end(), reverseMov),
        possible.end()
    );
    ...
    if (!possible.empty()) {
        ...
        std::mt19937 gen(rd());
        std::uniform_int_distribution<> dis(0, (int)possible.size() - 1);
        e_direction = possible[dis(gen)];
    }
...
bool Enemy::IsTileWalkable(int x, int y) const {
...
void Enemy::MoveY() {
    ...
    float nextY = enemyY + e_direction.y * ENEMY_SPEED;
    AABB futurePos = GetHitbox();
    futurePos.pos.y = (int)nextY;

    if ((e_direction.y < 0 && e_tileMap->TestCollisionWallUp(futurePos)) ||
        (e_direction.y > 0 && e_tileMap->TestCollisionWallDown(futurePos))) {
        return;
    }
